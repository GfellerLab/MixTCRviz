...
#'   If a column is missing, empty values will be used.
#'    * Other than .csv filename or data.frame, a third alternative consists of a list or .rds file with precomputed statistics, such as those generated by MixTCRviz in output.path/stats/.rds files.
#'  This last option assumes all TCR come from the same model.
#'  
...
#'   Other than .csv filename or data.frame, a third alternative consists of a list or .rds with precomputed statistics,
...
#'   
#' @param output.processed.data (default=FALSE) If T, Create a processed_data/ folder with the data for each model after the different processing steps
...
#' 
#' @param return.plot (default=FALSE): It TRUE, return the list of plots. 
#'    For optimal layout in Rstudio, use a command like `dev.new(width = 15, height = 8, reset = FALSE)` 
#'
#' @returns Nothing.
#' @export
MixTCRviz <- function(input1, output.path,
  ...
  if(!is.character(output.path) | length(output.path)>1){
    stop("Invalid value for output path. Should be a single string of character")
  }
  ...
  if(!is.logical(return.plot)){
    print("Invalid value for return.plot. Default value of FALSE will be used")
    return.plot <- F
  }
  if(return.plot & !plot){
    print("With the option return.plot=T, plot will be produced even if plot=F")
  }
