...
#' @param output.stat (default=F) If T, Create a stat/ folder with .rds objects summarizing the raw statistics for each model.
#'   This includes countL, countV, countJ, countCDR3.L, etc. for each chain used in input.
#' @param output.processed.data (default=F) If T, Create a processed_data/ folder with the data for each model after the different processing steps
...
MixTCRviz <- function(input1, output.path,
                      ...
                      renormVJ=1, N.min=10, output.stat=F, output.processed.data=F, 
  ...
  if(!renormVJ %in% c(0,1,2)){
    print(paste("renormVJ=",renormVJ," not supported, using default renormVJ=1", sep=""))
    renormVJ <- 1
  }
  ...
  if(!plot.cdr3.norm %in% c(0,1,2)){
    print(paste("plot.cdr3.norm=",plot.cdr3.norm," not supported, using default plot.cdr3.norm=0", sep=""))
    plot.cdr3.norm <- 0
  }
  ...
  if(is.null(input2)){
    ...
    comp.baseline <- 0
    if(renormVJ==1){ #Only renormVJ=2 forces the comparison with input2 | P(VJ)
      renormVJ <- 0
    }
    if(is.null(input2.name)){
  ...
  for(model in model.list){
    ...
    if(!input1.list){
      ...
      info <- list(model=model,species=sp)
      es <- append(es,info)
    ...
    if(plot==1){
      ...
      if(comp.baseline==1){
        ...
        if(!input2.list){
          ind <- which(input2[,"model"]==model & input2[,"species"]==sp)
          if(length(ind)==0){
          ...
          baseline <- input2
          if(input2$model != model){
            print("Warning: different model names between input1 and input2.")
            print("Model name in input2 will not be considered.")
          }
          if(input2$species != sp){
            stop("Different species between input1 and input2.")
          }
        }
      }
