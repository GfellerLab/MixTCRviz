...
#'     such as those generated by MixTCRviz::build_stat() function. This last option assumes all TCR come from the same model.
#'    * Other column names are supported, including for instance Va or V_alpha, 
#'    or v_call, vGeneName, v_resolved, allVHitsWithScore, V-region, v_gene when using chain.list.output="A" or "B", 
#'    following AIRR, MiXCR, 10X, Adaptive or Qiagen standards. Full mapping is described in data_raw/TidyVJ/mapping_colnames.csv.
#'    When the nomenclature does not contain indication about the chain, the chain is inferred from chain.output.list,
#'     and checked for compatibility with the actual V gene names.
...
MixTCRviz <- function(input1, output.path=NULL,
  ...
  if(is.character(input1)==T){
    ...
    stop("Invalid value for input1. Should be a .csv filename or a data.frame or a list generated by MixTCRviz")
  }
  
   
  chain.list.output <- verify.chain(input=input1, chain.list.output=chain.list.output)
  if(chain.list.output==""){
    stop("Incompatibilities between the data and chain.list.output, and unable to infer the chain... Maybe check chain.list.output parameter")
  }
  ...
  if(comp.baseline==0){
    ...
    input2 <- check_input(input=input2, chain.list.output=chain.list.output, name="input2", 
                          species.default = species.default, model.default = model.default, input.list=input2.list)$data
    if(!input2.list){
    ...
    if(plot){
      ...
      for(chain in chain.list){
        ...
        if(length(es$countL[[chain]])>0){
          ...
          if(print.size){
            info[2] <- paste(info[2], " (", sum(es$countL[[chain]]),")",sep="")
