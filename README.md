...
- input1: .csv files or data.frame with the input TCRs.
  ...
  * If TCRs from multiple models are provided in the same file, the "model" column should indicate the models
  ...
  * Other than .csv filename or data.frame, a third alternative consists of a list with precomputed statistics, such as those generated by MixTCRviz in output.path/stats/.rds files.
...
#'    This does not apply to the comparison with input2.
#'    * 2: Force the comparison with either baseline or input2 to include "| P(VJ)"
...
- output.stat (default=F) If T, Create a stat/ folder with .rds objects summarizing the raw statistics for each model. This includes countL, countV, countJ, countCDR3.L, etc. for each chain used in input.

- output.processed.data (default=F) If T, Create a processed_data/ folder with the data for each model after the different processing steps (e.g., removing alleles, correcting V/J names, removing inconsistent CDR3, etc. depending on the cleaning option selected).
...
- plot.VJ.switch (default=1)
    * 1: Show the VJ usage as a scatter plot with inner colors, outer colors and shapes of the points based on the V/J gene names (see lookup table in MixTCRviz/figures/, scheme1).
    * 1.2: Show the VJ usage as a scatter plot with colors of the points based on the V/J gene names (see lookup table in MixTCRviz/figures/, scheme2).
